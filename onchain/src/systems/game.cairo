...
pub mod Game {
    ...
    pub impl GameImpl of super::IGame<ContractState> {
        fn startPvpBattle(ref self: ContractState, enemyOwner: ContractAddress, heroesIds: Array<u32>) {
            ...
            let battleId = AccountsImpl::incrementAndGetLastBattleId(ref world, caller);
            ...
            BattlesImpl::newArenaBattle(ref world, caller, battleId, enemyOwner, allyEntities, enemyEntities);
        }
        fn playArenaTurn(ref self: ContractState, spellIndex: u8, targetIndex: u32) {
            let mut world = self.world(@"game");
            let battleId = AccountsImpl::incrementAndGetLastBattleId(ref world, get_caller_address());
            BattlesImpl::playArenaTurn(ref world, get_caller_address(), battleId, spellIndex, targetIndex);
        }
        fn startBattle(ref self: ContractState, heroesIds: Array<u32>, map: u16, level: u16) {
            ...
            BattlesImpl::newBattle(ref world, caller, battleId, allyEntities, enemyEntities, map, level);
        }
        fn playTurn(ref self: ContractState, map: u16, spellIndex: u8, targetIndex: u32) {
            let mut world = self.world(@"game");
            let battleId = AccountsImpl::incrementAndGetLastBattleId(ref world, get_caller_address());
            BattlesImpl::playTurn(ref world, get_caller_address(), battleId, map, spellIndex, targetIndex);
        }
